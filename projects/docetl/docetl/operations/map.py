...
Reference anchors:"""
    ...
    def execute(self, input_data: list[dict]) -> tuple[list[dict], float]:
        ...
        def _process_map_item(
            ...
            prompt = rendered
            custom_prompt = self.runner.config.get("custom_prompt", False)
            if custom_prompt:
                try:
                    operator_instruction = strict_render(self.config["prompt"], {})
                except UndefinedError as e:
                    raise ValueError(
                        "custom_prompt requires instruction-only templates; do not use {{ input ... }} in prompt."
                    ) from e
                context_text = context_text_from_item(
                    item,
                    self.runner.config.get("semantic_context_columns"),
                )
                op = (
                    SemanticOp.FILTER
                    if self.config.get("type") == "filter"
                    else SemanticOp.MAP
                )
                messages = build_semantic_prompt_messages(
                    operator_instruction, context_text, op
                )
            else:
            ...
            def _custom_prompt_output(response: ModelResponse) -> dict[str, Any]:
                if self.config.get("type") == "filter":
                    raw = (response.choices[0].message.content or "").strip().upper()
                    keep = raw.startswith("TRUE")
                    return {
                        self._filter_key if hasattr(self, "_filter_key") else "keep": keep
                    } 

                if len(output_schema) != 1:
                    raise ValueError(
                        "custom_prompt map expects exactly one output schema key for raw text output."
                    )
                key = next(iter(output_schema))
            ...
            def validation_fn(response: dict[str, Any] | ModelResponse):
                ...
                if custom_prompt and isinstance(response, ModelResponse):
                    output = _custom_prompt_output(response)
            ...
            if llm_result.validated:
                ...
                if isinstance(llm_result.response, ModelResponse):
                    ...
                    if custom_prompt:
                        outputs = [_custom_prompt_output(llm_result.response)]
                    else:
...
class ParallelMapOperation(BaseOperation):
    ...
    def execute(self, input_data: list[dict]) -> tuple[list[dict], float]:
        ...
        def process_prompt(item, prompt_config):
            ...
            # retrieval_context = self._maybe_build_retrieval_context({"input": item})
            # ctx = {"input": item, "retrieval_context": retrieval_context}
            # rendered = strict_render(prompt_config["prompt"], ctx)
            # prompt = (
            #     f"Here is some extra context:\n{retrieval_context}\n\n{rendered}"
            #     if retrieval_context
            #     and "retrieval_context" not in prompt_config["prompt"]
            #     else rendered
            # )
            ...
            if custom_prompt:
                try:
                    # operator_instruction = strict_render(
                    #     prompt_config["prompt"],
                    #     {"retrieval_context": retrieval_context},
                    # )
                    operator_instruction = strict_render(prompt_config["prompt"], {})
                except UndefinedError as e:
                    raise ValueError
                context_text = context_text_from_item(
                    item,
                ...
                messages = build_semantic_prompt_messages(
                    operator_instruction, context_text, SemanticOp.MAP
                )
            else:
                messages = [{"role": "user", "content": prompt}]
            if self.config.get("pdf_url_key", None):
