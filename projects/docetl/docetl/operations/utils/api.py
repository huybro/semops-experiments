...
class APIWrapper(object):
    ...
    def call_llm(
        ...
        use_custom_prompt = bool(self.runner.config.get("custom_prompt", False))
        if output_mode_str not in [mode.value for mode in OutputMode] and not use_custom_prompt:
            raise ValueError(
    ...
    def _call_llm_with_cache(
        ...
        if use_custom_prompt:
            # In semantic custom-prompt mode, always do plain text completion.
            # Parsing/validation is handled upstream per-operation.
            use_structured_output = False
            tools = None

        if use_custom_prompt:
            messages_with_system_prompt = truncate_messages(messages, model)
        else:
