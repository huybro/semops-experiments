...
from tdb.operators.prompt_utils import build_filter_messages
...
class UnaryFilter(SemanticOperator):
    ...
    def _evaluate_predicate_parallel(self, item_texts):
        ...
        for item_text, _, response in inputs_outputs:
            raw_result = str(response.choices[0].message.content or "").strip()
            normalized = raw_result.upper()
            keep = normalized.startswith("TRUE") or raw_result == "1"
            results.append((item_text, keep))
    ...
    def _message(self, item_text):
        ...
        """
        return build_filter_messages(self.filter_condition, item_text)
