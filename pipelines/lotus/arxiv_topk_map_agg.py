...
# Load Fever data
df = load_arxiv("/home/hojaeson_umass_edu/.cache/kagglehub/datasets/spsayakpaul/arxiv-paper-abstracts/versions/2/arxiv_50")
df = df.iloc[:10]
log = []
...
t0 = time.time()
input_len = len(df)
df = df.sem_topk(scenarios.CASE_1_TOPK_ARXIV, K=5)
df = df.sem_map(scenarios.CASE_1_MAP_ARXIV)
df = df.sem_agg(scenarios.CASE_1_AGG_ARXIV)
